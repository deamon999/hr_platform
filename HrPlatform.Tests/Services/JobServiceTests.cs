...
public class JobServiceTests
{
    ...
    public async Task GetAllAsync_ReturnsAllJobs_OrderedByPostedAtDesc()
    {
        ...
        using (var context = GetDbContext(dbName))
        {
            context.Jobs.AddRange(
                new Job { Id = 1, PostedAt = new DateTime(2022, 1, 1), CompanyId = 1 },
                new Job { Id = 2, PostedAt = new DateTime(2022, 1, 3), CompanyId = 1 },
                new Job { Id = 3, PostedAt = new DateTime(2022, 1, 2), CompanyId = 1 }
            );
    ...
    public async Task GetPagedAsync_FiltersAndSortsCorrectly()
    {
        ...
        {
            context.Jobs.AddRange(
                new Job { Id = 1, IsActive = true, PayRate = 50, RequiredCdlClass = CdlClass.A, PostedAt = new DateTime(2022, 1, 1), MinYearsExperience = 2, CompanyId = 1 },
                new Job { Id = 2, IsActive = false, PayRate = 60, RequiredCdlClass = CdlClass.B, PostedAt = new DateTime(2022, 1, 2), MinYearsExperience = 1, CompanyId = 1 },
                new Job { Id = 3, IsActive = true, PayRate = 70, RequiredCdlClass = null, PostedAt = new DateTime(2022, 1, 3), MinYearsExperience = 5, CompanyId = 2 }
            );
            await context.SaveChangesAsync();
        }
        ...
        using (var context = GetDbContext(dbName))
        {
            ...
            var driverProfile = new DriverProfile { License = new DriverLicense { Class = CdlClass.A }, YearsOfExperience = 3 };
    ...
    public async Task GetByIdAsync_ReturnsJob_WhenFound()
    {
        ...
        using (var context = GetDbContext(dbName))
        {
            context.Jobs.Add(new Job { Id = 1, CompanyId = 1 });
            await context.SaveChangesAsync();
        }
    ...
    public async Task CreateAsync_AddsJob()
    {
        ...
        {
            var service = new JobService(context);
            var result = await service.CreateAsync(new Job { CompanyId = 1, IsActive = true });
    ...
    public async Task UpdateAsync_UpdatesJob()
    {
        ...
        using (var context = GetDbContext(dbName))
        {
            context.Jobs.Add(new Job { Id = 1, PayRate = 10 });
            await context.SaveChangesAsync();
        }
    ...
    public async Task SetActiveAsync_UpdatesIsActive()
    {
        ...
        using (var context = GetDbContext(dbName))
        {
            context.Jobs.Add(new Job { Id = 1, IsActive = false });
            await context.SaveChangesAsync();
        }
